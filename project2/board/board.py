...
class Board:
    ...
    def check_winning_state_player_one(self):
        ...
        for node in reachable_nodes:
            for n in range(-1,2):
                if 0 <= node[1]+n<self.board_size and self.board[node[0]+1][node[1]+n] == 1:
                    if node[0]+1 == self.board_size-1:
                        return True
                    if (node[0]+1,node[1]+n) not in reachable_nodes: #Check if node is already added to avoid checking the same nodes twice
    ...
    def check_winning_state_player_two(self):
        ...
        for node in reachable_nodes:
            ...
            for n in range(-1,2):
                if 0 <= node[0]+n<self.board_size and self.board[node[0]+n][node[1]+1] == 2:
